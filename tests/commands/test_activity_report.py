...
from commands.activity_report import handle_activity_report
...
async def test_handle_activity_report_user_not_found(monkeypatch):
    ...
    monkeypatch.setattr("commands.activity_report.log_event", lambda *args, **kwargs: None)
...
async def test_handle_activity_report_success(monkeypatch):
    ...
    mock_interaction.user.display_avatar.url = "https://cdn.discordapp.com/avatars/user.png"
    mock_interaction.followup = MagicMock()
    mock_interaction.followup.send = AsyncMock()
...
async def test_handle_activity_report_exception_handling(monkeypatch):
    ...
    monkeypatch.setattr(
        "commands.activity_report.get_activity_worksheet", raise_error
    )
    monkeypatch.setattr("commands.activity_report.log_event", lambda *args, **kwargs: None)
