...
class DailyReportCron(commands.Cog):
    ...
    async def daily_task(self):
        ...
        try:
            ...
            report_data = await asyncio.to_thread(lambda: generate_daily_report(worksheet))

            if report_data:
                await broadcast_report_to_servers(self.bot, report_data)
                log_event(None, LogLevel.INFO, "Daily Report cron completed successfully.")
        except Exception as e:
            log_event(None, LogLevel.ERROR, f"Daily CRON Execution Error: {e}", exc=e)
