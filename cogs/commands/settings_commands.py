...
class SettingsCommandsCog(commands.Cog):
    ...
    @app_commands.command(name="set-cron-channel", description="Set the channel for cron reports")
    @app_commands.guild_only()
    async def set_cron_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        """Set the channel where cron reports will be posted for this server."""
        await handle_set_cron_channel(interaction, channel)

    @app_commands.command(name="remove-cron-channel", description="Remove the cron report channel setting")
    @app_commands.guild_only()
